...
public abstract class EntityVehicleBase extends Entity {
    ...
    protected void addPassenger(Entity passenger) {
        List<Entity> p = new ArrayList<>(getPassengers());
        p.add(passenger);
        ImmutableList<Entity> passengers = ImmutableList.copyOf(p);
    ...
    public void handledata() {
        if (isControlledByLocalInstance()) {
            // 发送运动数据到服务器
            PacketDistributor.SERVER.noArg().send(new ServerboundMoveVehiclePacket(this));
        }
    }
